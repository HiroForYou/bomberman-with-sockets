...
// hilo que dispara las siguientes coordenadas a los clientes mientras no se libera W/A/S/D
class CoordinatesThrower extends Thread {
   ...
   public void run() {
      ...
      while (true) {
         if (up || down || right || left) {
            if (up)
               newY = Server.player[id].y - Const.RESIZE;
            else if (down)
               newY = Server.player[id].y + Const.RESIZE;
            else if (right)
               newX = Server.player[id].x + Const.RESIZE;
            else if (left)
               newX = Server.player[id].x - Const.RESIZE;
   ...
   // encuentra en qué mapa sprites se encuentra el jugador y comprueba si son
   // válidos
   boolean coordinateIsValid(int newX, int newY) {
      if (!Server.player[id].alive)
         return false;

      // comprueba si el jugador entró en el fuego (coordenada del centro del cuerpo)
      ...
      if (Server.map[getLineOfMap(yBody)][getColumnOfMap(xBody)].img.contains("explosion")
            || Server.map[getLineOfMap(yBody)][getColumnOfMap(xBody + 50)].img.equals("enemigo")
            || Server.map[getLineOfMap(yBody)][getColumnOfMap(xBody - 50)].img.equals("enemigo")
            || Server.map[getLineOfMap(yBody + 50)][getColumnOfMap(xBody)].img.equals("enemigo")
            || Server.map[getLineOfMap(yBody - 50)][getColumnOfMap(xBody)].img.equals("enemigo")) {
      ...
      if ((Server.map[l[0]][c[0]].img.equals("floor-1") || Server.map[l[0]][c[0]].img.contains("explosion"))
         ...
         return true; // estará en una coordenada válida

      if ((Server.map[l[0]][c[0]].img.contains("block") || Server.map[l[0]][c[0]].img.contains("wall"))
         ...
         return false; // estará en una pared

      // EN RELACIÓN CON LA COORDENADA ANTERIOR
      ...
      if (Server.map[l[0]][c[0]].img.contains("bomb-planted") || Server.map[l[1]][c[1]].img.contains("bomb-planted")
            || Server.map[l[2]][c[2]].img.contains("bomb-planted")
            || Server.map[l[3]][c[3]].img.contains("bomb-planted"))
         return true; // estaba en una bomba que acabas de colocar, necesita salir
